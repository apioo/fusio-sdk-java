...
public class BackendUserTag extends TagAbstract {
    public BackendUserTag(HttpClient httpClient, ObjectMapper objectMapper, Parser parser) {
    ...
    public BackendUserCollection getAll(Integer startIndex, Integer count, String search) throws ClientException {
        try {
            ...
            return this.httpClient.execute(request, response -> {
                if (response.getCode() >= 200 && response.getCode() <= 299) {
                    var data = this.parser.parse(EntityUtils.toString(response.getEntity()), new TypeReference<BackendUserCollection>(){});
     ...
     */
    public CommonMessage resend(String userId, Passthru payload) throws ClientException {
        try {
            Map<String, Object> pathParams = new HashMap<>();
            pathParams.put("user_id", userId);
            ...
            URIBuilder builder = new URIBuilder(this.parser.url("/backend/user/$user_id<[0-9]+|^~>/resend", pathParams));
            this.parser.query(builder, queryParams, queryStructNames);
