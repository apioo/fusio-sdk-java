...
public class BackendAgentMessageTag extends TagAbstract {
     ...
     */
    public BackendAgentMessage submit(String agentId, BackendAgentContent payload) throws ClientException {
        try {
            Map<String, Object> pathParams = new HashMap<>();
            pathParams.put("agent_id", agentId);
            ...
            return this.httpClient.execute(request, response -> {
                if (response.getCode() >= 200 && response.getCode() <= 299) {
                    var data = this.parser.parse(EntityUtils.toString(response.getEntity()), new TypeReference<BackendAgentMessage>(){});
