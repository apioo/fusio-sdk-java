...
public class BackendEventTag extends TagAbstract {
     ...
     */
    public BackendEventCollection getAll(Integer startIndex, Integer count, String search, Integer taxonomy) throws ClientException {
        try {
            Map<String, Object> pathParams = new HashMap<>();
            ...
            return this.httpClient.execute(request, response -> {
                if (response.getCode() >= 200 && response.getCode() <= 299) {
                    var data = this.parser.parse(EntityUtils.toString(response.getEntity()), new TypeReference<BackendEventCollection>(){});
