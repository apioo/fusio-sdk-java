...
public class ConsumerScopeTag extends TagAbstract {
    public ConsumerScopeTag(HttpClient httpClient, ObjectMapper objectMapper, Parser parser) {
        ...
        try {
            ...
            return this.httpClient.execute(request, response -> {
                if (response.getCode() >= 200 && response.getCode() <= 299) {
                    var data = this.parser.parse(EntityUtils.toString(response.getEntity()), new TypeReference<ConsumerScopeCollection>(){});
    ...
    public ConsumerScopeCategories getCategories() throws ClientException {
        try {
            ...
            URIBuilder builder = new URIBuilder(this.parser.url("/consumer/scope/categories", pathParams));
            this.parser.query(builder, queryParams, queryStructNames);
            ...
            return this.httpClient.execute(request, response -> {
                if (response.getCode() >= 200 && response.getCode() <= 299) {
                    var data = this.parser.parse(EntityUtils.toString(response.getEntity()), new TypeReference<ConsumerScopeCategories>(){});
